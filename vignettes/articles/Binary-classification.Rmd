...
```{r}
df_lr <- apply_logistic_regression(df)
...
```{r}
df_lr2 <- apply_logistic_regression(df, order = 4, stepwise = TRUE)
...
```{r}
df_rt <- apply_classification_tree(df)

plot(df_rt)

plot(apply_classification_tree(df, alpha = 0.25))
```
...
```{r}
df_rt_response <- apply_classification_tree(df, type = "response")
...
```{r}
df_rt_node <- apply_classification_tree(df, type = "node", maxdepth = 3)
...
purrr::map(neighbours, apply_knn, df = df) |> 
  purrr::map(plot) |> 
  purrr::reduce(`+`) +
  patchwork::plot_layout(guides = "collect") &
  theme_void() + theme(legend.position = "none")

purrr::map(neighbours, apply_knn, df = df, type = "response") |> 
  purrr::map(plot) |> 
  purrr::reduce(`+`) +
  patchwork::plot_layout(guides = "collect")  &
  theme_void() + theme(legend.position = "none")
