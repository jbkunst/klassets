...
df2_2 <- sim_quasianscombe_set_2(df, fun = funktion)
```
...
dfs <- dfs |> 
  mutate(
    set = str_replace_all(set, "Set 2", "No linear relationship"),
    set = str_replace_all(set, "Set 3", "Extreme values (outliers)"),
    set = str_replace_all(set, "Set 4", "Cluster"),
    set = str_replace_all(set, "Set 5", "Heteroskedasticity"),
    set = str_replace_all(set, "Set 6", "Simpson's Paradox"),
    set = forcats::fct_inorder(set)
    )

gga <- ggplot(dfs, aes(x, y)) +
  geom_point(shape = 21, fill = "gray80", color = "gray60") +
  geom_smooth(method = "lm", se = FALSE, formula = y ~ x) +
  facet_null() +
   # then animate
  labs(
    ...
    caption = "Quasi Anscombe sets\n(only keep the regresion coefficients, and sometimes the mean of x's)"
    ) +
  ease_aes("cubic-in-out") +
  # shadow_wake(wake_length = 0.2, alpha = 0.1) +
  transition_states(set, transition_length = 2, state_length = 1)
